/**
 * Generated by orval v7.16.0 🍺
 * Do not edit manually.
 * Ferretería API
 * API REST para gestión de ferretería con soporte multi-tenant.
      
**Características:**
- Multi-tenancy por subdominio
- Autenticación JWT
- Gestión completa de inventario
- Control de caja y ventas
- Integración SUNAT
- Auditoría completa

**Autenticación:**
Todos los endpoints (excepto /auth) requieren token JWT en header Authorization: Bearer <token>
 * OpenAPI spec version: 2.0.0
 */
import type { UploadImagenProductoProductoImagenUrl } from "./uploadImagenProductoProductoImagenUrl";
import type { UploadImagenProductoProductoThumbnailSmall } from "./uploadImagenProductoProductoThumbnailSmall";
import type { UploadImagenProductoProductoThumbnailMedium } from "./uploadImagenProductoProductoThumbnailMedium";

/**
 * Datos del producto actualizado
 */
export type UploadImagenProductoProducto = {
  id: number;
  nombre: string;
  /** URL de la imagen original */
  imagen_url: UploadImagenProductoProductoImagenUrl;
  /** URL del thumbnail pequeño (100x100) */
  thumbnail_small?: UploadImagenProductoProductoThumbnailSmall;
  /** URL del thumbnail mediano (300x300) */
  thumbnail_medium?: UploadImagenProductoProductoThumbnailMedium;
};
